...
Para conectar a matriz de led na <span style="color:purple">*FPGA*</span> o pino de data deve ser configurado no pin planer. Lembrando que a <span style="color:purple">*FPGA*</span> já tem um pino 5V e o ground.
...
Serão necessário 4 códigos em <span style="color:blue">*vhdl*</span>:
...
Um arquivo <span style="color:green">.mif</span> deve ser criado para que o periférico seja incializado corretamente. Se esse arquivo for modificado antes da programação da placa os leds devem ficar acesos logo na inicialização.
...
A memória criada acima foi modificada para aceitar leitura e escrita simultanemante. O código pronto se encontra abaixo, junto com os outros <span style="color:blue">*vhdls*</span> necessários para a implementação do periférico.
...
::: info O que temos em cada arquivo
### Máquina de estados:
* **pixel_count**: A quantidade de pixeis que serão modificados. No nosso caso o total da matriz.
* **clock_frequency**: A frequência do clock da <span style="color:purple">*FPGA*</span> (para ser utilizado nos cálculos de espaço de tempo entre os bits 1 e 0 para representar a informação que será recebida pela matriz)
...
Na aba signals coloque **conduit_end** na serial_out para que esse sinal seja reconhecido como uma saída do periférico na <span style="color:purple">*FPGA*</span>.
...
O componente aparecerá na janela **IP Catalog**. Um arquivo <span style="color:green">*.tcl*</span> deve ter sdo criado na pasta raíz do projeto. Leve esse arquivo para a pasta IP e modifique-o para que o caminho até o componente esteja correto.
...
Clique em **Generate HDL** --> **Generate** para gerar o componente e use a opção **create simulation model** com <span style="color:blue">*vhdl*</span> para que seja criado os arquivos auxiliares configurados de acordo com o que está no platform designer.
...
Depois de criar o novo arquivo de top level compile o projeto e programe a <span style="color:purple">*FPGA*</span>.
...
O componente **WS2812** deve ler os endereços da memória, processar a informação como descrito nos códigos em <span style="color:blue">*vhdl*</span> e passar os dados para a matriz de led pelo serial. A NIOS deve ser capaz de modificar a ram com o wren (permitindo escrita), data (o dado a ser Escrito) e address (o endereço que deve ser moficado).
