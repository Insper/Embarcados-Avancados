...

{
   ...
   * @param  {string} options.href If defined, the notification contains an anchor
   */
  notification.toast = function (text, options) {
    options = options || {};
    let { timeout, color, bgColor, href } = options;
    ...
    const contentTag = href !== undefined ? "a" : "span";
    const contentAttrs = href !== undefined ? `href="${href}"` : "";
    toast.innerHTML = `
      <${contentTag} ${contentAttrs} class="toast-content">
        ${text}
      </${contentTag}>
      `;
